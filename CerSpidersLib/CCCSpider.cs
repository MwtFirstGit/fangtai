...
namespace CerSpidersLib
{
    public class CCCSpider : CerSpiderBase
    {
        ...
        /// CCC证件Url
        /// </summary>
        const String CCC_Details_Url = "http://webdata.cqccms.com.cn/webdata/query/CCCCerti.do";
        /// <summary>
        /// CCCPost请求参数
        /// </summary>
        const String CCC_PostData = "keyword={0}&_h_select_chaxuntype=certinum&chaxuntype=certinum&imagePassword={1}";
        /// <summary>
        /// xpath获取标题
        /// </summary>
        ...
        private Dictionary<String, String> CCC_Details(String Certi_No)
        {
            Dictionary<String, String> dirs = new Dictionary<string, string>();
            HttpInfo info = new HttpInfo();
            String html = "start";
            String code = String.Empty;
            String cookie = Get_Cookie(CCC_Details_Url);
            try
            {
                while (!html.Contains(Certi_No) || html.Contains("验证码输入有误") || html.Contains("请输入验证码"))
                {
                    code = WmCodeHelper.Get_Code(cookie);
                    info.RequestUrl = CCC_Details_Url;
                    info.PostData = String.Format(CCC_PostData, Certi_No, code);
                ...
                if (html.Contains(Certi_No))
                {
                    ...
                    string fjurl = XpathMethod.GetSingleResult(xpath_certi + "[15]", html, 0);
